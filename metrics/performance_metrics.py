...
class PerformanceMetrics:
    ...
    def perform_evaluation(self, genuine_similarity_list: List[float], impostor_similarity_list: List[float]) -> Tuple[float, float, List[float], List[float]]:
        """计算等错误率(EER)和最佳阈值
        ...
        Returns:
            Tuple[float, float, List[float], List[float]]: EER, 最佳阈值, FAR列表, GAR列表
        """
        assert len(genuine_similarity_list) > 0 or len(impostor_similarity_list) > 0, "No similarity data provided"
        best_eer, best_thrshold, far_list, gar_list = CalculateVerificationRate.computePerformance(
